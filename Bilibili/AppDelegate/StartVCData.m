...
-(void)download_startView_data{
    //获取宽高的分辨率
    CGFloat width = CGRectGetWidth([[UIScreen mainScreen] bounds]) ;  //[[UIScreen mainScreen] bounds].size.width;
    
    CGFloat height = CGRectGetHeight([[UIScreen mainScreen] bounds]);   //[[UIScreen mainScreen] bounds].size.height;
    
    CGFloat scale_screen = [UIScreen mainScreen].scale;  //这个不是缩放系数，是屏幕分辨率，iphone4的分辨率是640＊960的，之前的版本是320＊480的     在6s上运行打印的该值是2
    ...
    [[session dataTaskWithRequest:request completionHandler: ^(NSData *data, NSURLResponse *response, NSError *error)
    {
        if (!error) {
            ...
            //预加载一遍视图
            NSDictionary* startView_data_dic = [NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingMutableContainers error:nil];
            NSLog(@"首页启动数据 %@",startView_data_dic);
